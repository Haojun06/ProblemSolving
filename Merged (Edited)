    ...
    if os.path.exists("mathscore.json"):# If the score file exists
        ...
        if data: # If there are scores
            ...
            for i, entry in enumerate(top_scores):
                print(f"{i+1}. {username}: {entry['score']}")
        else:
...
def math_quiz(username):
    ...
    if os.path.exists("mathcore.json"):
        try:
            with open("mathcore.json", "r") as file: #open the file in read mode
                data = json.load(file)# Load existing scores
...
def load_anagram_scores():
    try:
        with open("anagram_scores.json", "r") as file:  #open the scores.json file in read mode
...
#Function to show leaderboard before start
def show_bombleaderboard(username):
